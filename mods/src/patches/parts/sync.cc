...
{
  const auto& cfg = Config::Get();
  if (!cfg.raw_static_capture_event_model_rewards) {
    return;
  }

  if (cfg.raw_static_capture_directory.empty()) {
    spdlog::error("sync.raw_static_capture.event_model_rewards is true, but sync.raw_static_capture.directory is empty");
    return;
  }

  const auto root = std::filesystem::path(cfg.raw_static_capture_directory);
...
static void write_worker()
{
  while (true) {
    ...
    const auto& cfg = Config::Get();
    if (!cfg.raw_static_capture_event_model_rewards || cfg.raw_static_capture_directory.empty()) {
      continue;
    }
...
void EventModel_UpdateCurrentRewardBundle(auto original, void* _this, void* bundles_by_id)
{
  ...
  if (!Config::Get().raw_static_capture_event_model_rewards_full) {
...
static void InstallEventModelRewardBundleHook()
{
  ...
  if (cfg.raw_static_capture_directory.empty()) {
    spdlog::error("Not installing EventModel reward-bundle logger: sync.raw_static_capture.directory is empty");
    return;
  }
